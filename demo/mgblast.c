...
/* Geo's new callback output functions:

 MegaBlastPrintFltHits(VoidPtr ptr)
 MegaBlastPrintGapHits(VoidPtr ptr) 
...
#else
/* -- end of mgblast clustering features -- */
#ifdef MB_ALLOW_NEW
 ...
 
{
   ...
   Boolean traditional_formatting;
    ...
    /* Geo add-on -- we NEVER care about NCBI's defline parsing! */
    believe_query = FALSE;
	...
	while (!done) {
	   ...
	   error_returns = NULL;
	      ...
	      seqalign_array = BioseqMegaBlastEngine(query_bsp_array, blast_program,
              ...
              /* why there is no option to disable these headers?!
...
{ 
   ...
   while (1) {
      ...
      if (tabular_output) {
          ...
          tf_data = BlastTabularFormatDataNew(outfp, query_slp, tab_option, believe_query);
